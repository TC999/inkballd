...
#include <windows.h>
#include "common.h"
BOOL __stdcall Helpers::PostMessageW(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam, uint32_t *a5, int*a6)
{
  BOOL v6; // esi
  ...
  *(uint32_t *)v10 = 0;
  Helpers::CLogBlock::CLogBlock((Helpers::CLogBlock *)v9, "Helpers::PostMessageW", (int*)v10);
  ...
  if ( !v6 )
  {
    ...
    *(uint32_t *)v10 = LastError;
    if ( WPP_GLOBAL_Control != &WPP_GLOBAL_Control && (*((uint8_t *)WPP_GLOBAL_Control + 28) & 4) != 0 )
      WPP_SF_d(*((uint64_t *)WPP_GLOBAL_Control + 2), 0x37u, &stru_10036F8, v10[0]);
  }
  if ( a5 )
    *a5 = *(uint32_t *)v10;
