void __thiscall CBoardManager::LoadRandomBonusBoardFromResources(CBoardManager *this, unsigned __int8 *Src, int *a3)
{
  ...
  Helpers::CLogBlock::CLogBlock((Helpers::CLogBlock *)v9, "CBoardManager::LoadRandomBonusBoardFromResources", 0);
  v11 = 0;
  do
    RandomNumber = GetRandomNumber(iBonusBoardListCount);
  while ( fPrevSeen(RandomNumber, (struct BOARDHIST *)&g_bhPrevBonusBoards, iBonusBoardListCount) );
  v4 = (unsigned __int8 *)&g_BoardData + *((_DWORD *)paBonusBoardList + 129 * RandomNumber);
  v5 = *v4++ << 8;
  v6 = *v4 + v5;
  memcpy(Src, v4 + 1, v6);
  if ( v6 <= 0x1000 )
