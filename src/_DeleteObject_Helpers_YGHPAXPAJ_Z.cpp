...
#include <windows.h>
#include "common.h"
BOOL __stdcall Helpers::DeleteObject(HGDIOBJ ho, uint32_t *a2, int*a3)
{
  BOOL v3; // esi
  ...
  *(uint32_t *)v7 = 0;
  Helpers::CLogBlock::CLogBlock((Helpers::CLogBlock *)v6, "Helpers::DeleteObject", (int*)v7);
  v3 = DeleteObject(ho);
  if ( !v3 )
  {
    ...
    *(uint32_t *)v7 = LastError;
    if ( WPP_GLOBAL_Control != &WPP_GLOBAL_Control && (*((uint8_t *)WPP_GLOBAL_Control + 28) & 4) != 0 )
      WPP_SF_d(*((uint64_t *)WPP_GLOBAL_Control + 2), 0xEu, &stru_10036F8, v7[0]);
  }
  if ( a2 )
    *a2 = *(uint32_t *)v7;
