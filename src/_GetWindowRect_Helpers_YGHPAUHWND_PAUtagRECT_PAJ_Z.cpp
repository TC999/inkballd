...
#include <cstdlib>
#include <windows.h>
BOOL __stdcall GetWindowRect_Helpers(Helpers* self, HWND hWnd, RECT *lpRect, RECT *a3, int*a4)
{
  BOOL WindowRect; // esi
  ...
  *(uint32_t *)v8 = 0;
  Helpers::CLogBlock::CLogBlock(reinterpret_cast<Helpers::CLogBlock*>(v7), "Helpers::GetWindowRect", 0);
  ...
  if ( !WindowRect )
  {
    ...
    *(uint32_t *)v8 = LastError;
    if ( WPP_GLOBAL_Control != reinterpret_cast<uint64_t>(&WPP_GLOBAL_Control) && (reinterpret_cast<uint8_t*>(static_cast<uintptr_t>(WPP_GLOBAL_Control))[28] & 4) != 0 )
      WPP_SF_d(*reinterpret_cast<uint64_t*>(static_cast<uintptr_t>(WPP_GLOBAL_Control)) + 2, 0x21u, &stru_10036F8, v8[0]);
  }
  if ( a3 )
    a3->left = *(uint32_t *)v8;
