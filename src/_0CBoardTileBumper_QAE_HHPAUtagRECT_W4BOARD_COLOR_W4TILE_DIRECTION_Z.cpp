...
extern "C" {
    CBoardTile* CBoardTileBumper::CBoardTileBumper(CBoardTile* this_ptr, int x, int y, int rect_param, int color, int direction)
    {
      uint8_t log_buffer[8]; // [esp+4h] [ebp-8h] BYREF

      CBoardTile::CBoardTile(this_ptr);
      *reinterpret_cast<uint32_t*>(this_ptr) = reinterpret_cast<uint32_t>(&CBoardTileBumper::`vftable`);
      Helpers::CLogBlock::CLogBlock(reinterpret_cast<Helpers::CLogBlock*>(log_buffer), "CBoardTileBumper::CBoardTileBumper", 0);
      ...
      *reinterpret_cast<uint32_t*>(this_ptr + 17) = 11;
