...
#include <cstdlib>
#include <windows.h>
int ConvertGDIColor_CDisplay(CDisplay* self, COLORREF color)
{
  int v3; // edi
  bool v4; // zf
  int v6; // eax
  ...
  if ( !v4 )
  {
    ...
    if ( color != -1 )
    {
      ...
      else
      {
        ...
        (*(void (__stdcall **)(uint32_t, HDC))(**((uint32_t **)self + 5) + 104))(*((uint32_t *)self + 5), hdc[0]);
      }
    }
    v7 = *((uint32_t *)self + 5);
    v9[0] = 124;
    if ( !(*(int (__stdcall **)(int, uint32_t, uint32_t *, int, uint32_t))(*(uint32_t *)v7 + 100))(v7, 0, v9, 1, 0) )
    {
      ...
      if ( v10 < 0x20 )
        v13 &= (1 << v10) - 1;
      (*(void (__stdcall **)(uint32_t, uint32_t))(**((uint32_t **)self + 5) + 128))(*((uint32_t *)self + 5), 0);
    }
    if ( color != -1 )
    {
