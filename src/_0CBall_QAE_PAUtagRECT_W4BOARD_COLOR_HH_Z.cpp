...
int CBall::CBall(int this_ptr, int rect_x, int rect_y, int should_add, int ball_radius)
{
    struct CBallLayout {
    ...
    CMovingObject::CMovingObject(reinterpret_cast<CMovingObject*>(this_ptr));
    auto layout = reinterpret_cast<CBallLayout*>(this_ptr);
    layout->vftable_ptr = reinterpret_cast<uint32_t>(&CBall::`vftable');
    
    Helpers::CLogBlock::CLogBlock(reinterpret_cast<Helpers::CLogBlock*>(log_buffer), "CBall::CBall", 0);
    log_state = 0;
    layout->field_136 = 0;
    log_state = 1;
    ...
    log_state = 2;
    if (ball_points_mem)
        ball_points = BallPoints::BallPoints(reinterpret_cast<BallPoints*>(ball_points_mem), ball_radius);
    else
        ball_points = nullptr;
    layout->field_124 = static_cast<uint32_t>(-1);
    layout->field_120 = reinterpret_cast<uint32_t>(ball_points);
