...
BOOL __stdcall DeleteDC_Helpers(void* self, HDC hdc, HDC a2, int*a3)
{
  ...
  *(uint32_t *)v7 = 0;
  Helpers::CLogBlock::CLogBlock(reinterpret_cast<Helpers::CLogBlock*>(v6), "Helpers::DeleteDC", 0);
  v3 = ::DeleteDC(hdc);
  if ( !v3 )
  {
    ...
    *(uint32_t *)v7 = LastError;
    if ( WPP_GLOBAL_Control != reinterpret_cast<uint64_t>(&WPP_GLOBAL_Control) && (reinterpret_cast<uint8_t*>(static_cast<uintptr_t>(WPP_GLOBAL_Control))[28] & 4) != 0 )
      WPP_SF_d(*reinterpret_cast<uint64_t*>(static_cast<uintptr_t>(WPP_GLOBAL_Control)) + 2, 0xFu, &stru_10036F8, v7[0]);
  }
  if ( a2 )
